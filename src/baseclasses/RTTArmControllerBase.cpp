...
bool RTTArmControllerBase::updateDynamicsAndKinematics(
		const rstrt::kinematics::JointAngles& jPosFb, const rstrt::kinematics::JointVelocities& jVelFb,
		const rstrt::dynamics::JointTorques& jTrqFb) {

	if (!kdlFieldsInitialized || !properlyParsed) {
		return false;
	}

	jnt_pos_ = jPosFb.angles.cast<double>();
	jnt_vel_ = jVelFb.velocities.cast<double>();
	jnt_trq_ = jTrqFb.torques.cast<double>();
...
RTTArmControllerBase::RTTArmControllerBase(std::string const& name,
		...
		RTTJointAwareTaskContext(name), DEFAULT_ROOT_LINK(default_root_link), DEFAULT_TIP_LINK(
